"""
Diffusion Transformer (DiT) for Flow Matching — Lumina-T2I Architecture.

Based on Lumina-T2X Flag-DiT (Alpha-VLLM, 2024), adapted for PICFM topology
optimization with SpatialStyleEncoder (v2).

Architecture (follows Lumina-T2I):
  - RMSNorm (instead of LayerNorm)
  - SwiGLU FFN (3-matrix gated)
  ...
  - attention_y_norm: RMSNorm on context before cross-attention K/V
...
from flows.design_encoder import SpatialStyleEncoder
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # Tanh-gated cross-attention (zero-init → starts silent)
...
class FinalLayer(nn.Module):
    """Final layer: adaLN modulation (zero-init) → LayerNorm → Linear (zero-init)."""
...
class DiTModel(nn.Module):
    ...
    def __init__(
        ...
        # ---- Cap embedder (Lumina: LN → Linear, zero-init) ----
    ...
    def forward(self, t, x, cond_spatial=None, use_null=False, null_mask=None, disable_ds=False):
        """
        Args:
            t: (B,) or scalar — diffusion timesteps
            x: (B, 1, H, W) — noisy image
            cond_spatial: (B, 10, H, W) — condition tensor
            use_null: bool — use null tokens for all samples (CFG uncond pass)
            null_mask: (B,) bool — per-sample null mask (CFG dropout)
            disable_ds: bool — zero out DS stream (BC-only ablation)
        Returns:
            (B, 1, H, W) — predicted velocity field
        """
        while t.dim() > 1:
            t = t[:, 0]
        if t.dim() == 0:
    ...
    def freeze_backbone_for_stage2(self):
        """Freeze self-attention + FFN backbone, keep cross-attention + encoder DS trainable.

        Trainable after freeze:
          - Cross-attention: wk_y, wv_y, gate, ky_norm (per block)
          - attention_y_norm (per block)
          - cap_embedder
          - Encoder DS stream: ds_class_embeddings, ds_refine, ds_pos_embed
          - Encoder fusion: fusion_norm, fusion_proj
          - Encoder null_context
        """
        ...
        for layer in self.layers:
            ...
            if hasattr(attn, "wk_y"):
                for p in attn.wk_y.parameters():
                    p.requires_grad_(True)
                for p in attn.wv_y.parameters():
                    p.requires_grad_(True)
                attn.gate.requires_grad_(True)
                if hasattr(attn, "ky_norm") and not isinstance(attn.ky_norm, nn.Identity):
                    for p in attn.ky_norm.parameters():
                        p.requires_grad_(True)
            for p in layer.attention_y_norm.parameters():
                p.requires_grad_(True)

        # Unfreeze cap_embedder
        for p in self.cap_embedder.parameters():
            p.requires_grad_(True)

        # Unfreeze encoder DS stream + fusion
        enc = self.cond_encoder
        enc.ds_class_embeddings.requires_grad_(True)
        enc.ds_pos_embed.requires_grad_(True)
        enc.null_context.requires_grad_(True)
        for p in enc.ds_refine.parameters():
            p.requires_grad_(True)
        for p in enc.fusion_norm.parameters():
            p.requires_grad_(True)
        for p in enc.fusion_proj.parameters():
            p.requires_grad_(True)
